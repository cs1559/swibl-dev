...
class JLHtml {
	...
	function getParentDivisionList($element_name, $default_value, $season, $event = '') {
		$db			=& JLApp::getDBO();
		if ($season > 0) {
			$query = "select id as value, name as text from #__jleague_division where published = 1 and parent_indicator = 1 and season = " . $season . " order by sort_order";
		} else {
			$query = "select id as value, name as text from #__jleague_division where published = 1 and parent_indicator = 1 and season = 999999 order by sort_order";
		}
		$db->setQuery($query);
		$rows = $db->loadObjectList();
		// Create an empty list option
		$categorylist[] =JHTML::_('select.option', '', '-- Select Parent Divsion --       ' );
