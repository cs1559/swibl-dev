...
require_once(JLEAGUE_CLASSES_PATH . DS. 'factory.php');
require_once(FST_LIB_CORE . DS . 'basedao.php');

//class JLRegistrationDAO extends JLBaseDAO{
class JLRegistrationDAO extends fsBaseDAO {
	 ...
	 */
   	function insert(JLTeamRegistration $obj) {
		$query = 'INSERT INTO ' .$this->getNameQuote( $this->tablename  ) . ' (id, division_id, season, team_id, name, address, city, state, email, teamname, agegroup, existingteam, phone, cellphone, published, paid, confirmed, tournament, regdate, registeredby, divclass, confnum) '
			. ' VALUES (0,'
			. '"' . $obj->getDivisionId() . '",' 
			. '"' . $obj->getSeasonId() . '",'
			. '"' . $obj->getTeamId() . '",'
