...
from #__jleague_teams t, #__jleague_division d, #__jleague_divmap dm
...
order by t.name, d.agegroup";
	...
	static function getTeamContactRolesList($element_name, $default_value, $classname="select-contact-roles", $event = '') {
		...
		$obj->addOption("All-Star Contact","All-Star Contact");
		$obj->addOption("Other","Other");
		$obj->setAttribute("class", $classname);
		return $obj->toHtml();
	}
	...
	static function getVenueSelectList($element_name, $default_value,  $classname="select-venue", $event = "") {
		...
		$rows = $svc->getVenues();
		$obj = new fsSelectList($element_name, $default_value,$element_name,$classname);
...
// 			$field = $venue->getName() . " - " . $venue->getCity();
			...
			$html .= "<li onClick=\"javascript: setLocation('" . $field ."');\">" . $field . "</li>";
		}
		$html .= "</ul>";
		return $html;
	}
	...
	static function getRegistrationOpenSeasonRadio($element_name, $default_value, $classname = "registrationOpenRadioButton", $getall = false, $event = null) {
		$db			=& mFactory::getDBO();
		// 		$query = "select id as value, concat(name,' ( ',username,' )')  as text from #__users order by username";
		$query = "select id as value, title as text from #__jleague_seasons where published = 1 and registrationopen = 1";
		$db->setQuery($query);
		...
		foreach ($rows as $row) {
			//$obj->addOption($row->value, $row->text);
			$html .= "<input type='radio' name='" . $element_name . "' value='" . $row->value ."'>&nbsp;" . $row->text . "<br/>";
		}
...
// 		$obj->setAttribute("class", $classname);
	...
	static function getFallAgeGroupSelectList($element_name, $default_value, $event = '') {
		$default_value = str_replace('U','',$default_value);
		$obj = new fsSelectList($element_name, $default_value);
		$obj->setHeader("---- Select Age ----");
		$obj->addOption("7", "7 and Under");
	...
	static function getDivisionClassList($name, $default_value=null) {
		...
		$obj->addOption("R","Red - Typically Class AA or higher teams");
		$obj->addOption("W","White - Typically Class A");
		$obj->addOption("B","Blue - First year Teams coming from Rec Ball");
		return $obj->toHtml();
	}
	
	
	static function getInputElement($name,$value,$size=30,$length=30) {
		if (!strlen($size) > 0) {
			$size = 30;
		}
		if (!strlen($length) > 0) {
	...
	static function getTextAreaElement($name,$value,$rows=30,$cols=30) {
		if (!strlen($rows) > 0) {
			$rows = 30;
		}
		if (!strlen($cols) > 0) {
