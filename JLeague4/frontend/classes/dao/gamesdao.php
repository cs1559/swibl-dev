 	...
 	function getTeamGames($teamid=null, $seasonid= null, $statusfilter=null, $sortorder = "desc") {
		...
		if ($statusfilter == "all") {
   		...
   		$query = " select * from (SELECT g.*, t1.name hometeam, t2.name awayteam "
				...
				. " order by game_date asc, gametime asc";	
...
SELECT s.*,t1.logo as hometeam_logo, t2.logo as awayteam_logo
from #__jleague_scores s
left join #__jleague_teams as t1
on s.hometeam_id = t1.id
left join #__jleague_teams as t2
on s.awayteam_id = t2.id
where (hometeam_id = 272 or awayteam_id = 272)
and gamestatus = 'S' and conference_game = "Y"
and game_date >= curdate() 
order by season desc, game_date 

  	 */
	  	$query = "SELECT s.*,t1.logo as hometeam_logo, t2.logo as awayteam_logo "
	  		. " from #__jleague_scores s "
			. " left join #__jleague_teams as t1 "
			. " on s.hometeam_id = t1.id "
			. " left join #__jleague_teams as t2 "
...
// 		require_once(JLEAGUE_CLASSES_PATH . DS .'helpers'. DS . 'util.php');
		...
		$obj->setHometeamLogo($row->hometeam_logo);
		$obj->setAwayteamId($row->awayteam_id);
		$obj->setAwayteamLogo($row->awayteam_logo);
