...
// 		require_once(JLEAGUE_SERVICES_PATH . DS . 'seasonservice.class.php');
		...
		try {
			...
			$record->setTeamName("");
			$record->setDivisionId(0);
			$record->setDivisionName("Unassigned");
			$record->setSeasonId(0);
			$record->setSeason($season->getTitle());
			$record->setWins(0);
			$record->setLosses(0);
			$record->setTies(0);
			$record->setRunsScored(0);
			$record->setRunsAllowed(0);
			//return $record;
			//throw $e;
		}
		return $record;
	...
	public function getActiveTeams() {
		...
		$cache_key = 'getTeamsInSeason';
		try {
			$rows = $cache->get($cache_key, $filter_season);
		} catch (Exception $e) {
...
//		$rows = $dao->getTeamsInSeason($filter_season,1);
	 ...
	 * The filterd attribute will filter any ALL-STAR contacts
	 *
	 * @param int $teamid
	 * @return array
	 */
	public function getTeamContacts($teamid, $filtered=false) {
		if (!is_numeric($teamid)) {
			throw new  Exception("Team Id is not numeric");
		}			
		$dao = $this->getDao();
		return $dao->getTeamContacts($teamid,$filtered);
	}
...
// ===============================================================================================
    ...
    private function buildTeamView($teamid) {
			...
			$gamehistory = self::getTeamGames($teamid,$team->getSeason()->getId());
			//$schedule = self::getTeamSchedule($teamid,$team->getSeason()->getId());
			$schedule = self::getTeamSchedule($teamid,$seasonid);
			try {
			...
			$contacts = self::getTeamContacts($team->getId(), false);
    ...
    function validateTeamSchedule($teamid, $season) {
    	...
    	$required_games = $games->required_games;
     	//echo "games = " . $gamecount . "  required = " . $required_games;
     	if ($gamecount == 0) {
     		throw new Exception("No game(s) have been scheduled/entered");
     	}
    	...
    	if ($gamecount < $required_games) {
    		throw new Exception("Schedule has too few league games [" . $gamecount . "] scheduled than is required  [" . $required_games . "]");
    	}
    ...
    function calculateTeamStats($season) {
		...
		echo "# of teams to process: " . count($teams);
		foreach ($teams as $team) {
			try {
				echo "Processing team: " . $team->getName() . "<br/>";
				...
				$db->setQuery($query);
				if (!$db->query()) {
				throw new Exception("Error - failed processing " . $team->getName());
				}
				// 	    	$dao->query();

			} catch (Exception $e) {
				echo "Cannot process team stats";
			}
    ...
    /**
     * This function will execute the necssary rules to validate a team profile.
     * 
     * @param unknown $teamid
     ...
     */
    function validateTeamProfile($teamid, $seasonid) {
    	if (!is_numeric($teamid)) {
    		throw new  Exception("Team Id is not numeric");
    	}
    	$tid = (int) $teamid;
    	...
    	try {
    		$bool = $this->validateTeamSchedule($tid, $sid);
    	} catch (Exception $e) {
    		$issues[] = $e->getMessage();
    	}
    	...
    	// Validate Roster
    	$rsvc = &JLSimpleRosterService::getInstance();
    	$roster = $rsvc->getRoster($tid, $sid);
    	$players = $roster->getPlayerCount();
    	if ($players == 0) {
    		$issues[] = "There is currently no ROSTER defined";
    	} elseif ($players < 9) {
    		$issues[] = "Current roster does not have at least nine players";
    	}
    	
    	// Validate/Check for Primary Contact
    	$cdao = &JLTeamContactsDAO::getInstance();
    	try {
    		$bool = $cdao->isPrimaryContactDefined($tid);
    	} catch (Exception $e) {
    		throw $e;
    	}
    	if (!$bool) {
    		$issues[] = "No PRIMARY CONTACT defined in the Profile";
    	}
    	return $issues;
    }
    
    /**
     * This function will return an array of team information that has no identified owner. 
     */
    function getTeamsWithNoOwner( $seasonid) {
    	...
    	try {
    		$dao = &JLTeamDAO::getInstance();
    		$teams = $dao->getTeamsWithNoOwner($sid);
    	} catch (Exception $e) {
    		throw $e;
    	}
    	return $teams;
    ...
    /**
     * This function will return an array that includes the standings data as well as tournament and paid flags from their
     * registration.
     * 
     * @param int $seasonid
     * @throws Exception
     ...
     */
      	function getStandingsWithRegistrationData( $seasonid) {
        	...
        	try {
        		$dao = &JLTeamDAO::getInstance();
        		$teams = $dao->getStandingsWithRegistrationData($sid);
        	} catch (Exception $e) {
        		throw $e;
        	}
        	return $teams;
