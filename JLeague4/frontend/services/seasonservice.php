...
class JLSeasonService  extends mBaseService  {

	private $active_season = null;
	private $current_season = null;   //not sure I really nead this
	...
	*/
	...
	function getCurrentSeason() {
		...
		if ($this->current_season == null) {
			try {
				$dao = $this->getDao();
				$this->current_season = $dao->findById($seasonid);
			} catch (Exception $e) {
				throw $e;
			}
		}
    ...
    /**
     * This function will return an array containing the names of divisions (division_name) and # of teams (team_count)
     * @param unknown $id
     * @throws Exception
     * @return number
     */
    function getTotalTeamsByDivisionForSeason($id) {
    	...
    	try {
    		$divisions = $dao->getTotalTeamsByDivisionForSeason($id);
    		if (count($divisions) == 0) {
    			throw new Exception("No Divisions Found");
    		}
    		return $divisions;
    	} catch (Exception $e) {
    		throw $e;
