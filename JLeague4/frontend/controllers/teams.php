...
class mTeams extends mBaseController {
...
// 			//$filter = JLHtml::getSeasonSelectList('seasonid', $season->getId(),true,'onchange="getTeamsForSeason(this.value);"');
	...
	function viewTeamProfile() {
		...
		$teamid = self::getVariable("teamid");
		...
		$doc->setTitle("SWIBL - " . $team->getName() . " Profile");
		
		$view = new mTeamView(APP_TEMPLATES_PATH . DS . 'profiles'. DS . 'profile20' . DS);
		$view->addStylesheet(JURI::root() . 'components/com_jleague/templates/mobile/profiles/profile20/css/teamprofile.css');
		
		//$view->addStylesheet("assets/css/teamprofile.css");
		//$view->addStylesheet(APP_TEMPLATES_PATH . DS . 'profiles'. DS . 'profile20' . DS . 'css' . DS . 'teamprofile.css');
...
// 			$season = $app->getCurrentSeason();
			$profile_msgs = $svc->validateTeamProfile($teamid, $season->getId());
		} else {
			$submenu = null;
		}
		...
		$standings_tmpl->setObject("division",$tview->getDivision());
		$standings_tmpl->setObject('publishstandings',$season->getPublishStandings());
		...
		$tmpl->setObject("config",$config);
		$tmpl->setObject("app",$app);
		$tmpl->setObject('publishstandings',$season->getPublishStandings());
		...
		if (count($profile_msgs) > 0) {
			$tmpl->setObject("messages", $view->getIssuesHtml($profile_msgs));
		}
		...
		$vtmpl = new fsTemplate("teamprofile.view.venues");
		$vtmpl->setAlias("venuesHtml");
		$vtmpl->setObject("venues", $tview->getVenues());
		...
		try {
			$rtmpl = new fsTemplate("teamprofile.view.roster");
			$roster = $tview->getRoster();
			
			if (JLSecurityService::canViewRoster($team, $roster->getSeason())) {
				$players = $roster->getPlayers();
				$rtmpl->setAlias("rosterHtml");
				$rtmpl->setObject("players", $players);
				$tmpl->addTemplate($rtmpl);
			} 
		} catch (Exception $e) {
			//$tmpl->setObject('rosterhtml','Roster Unavailable');
		}

		$tmpl->addTemplate($standings_tmpl);
		$tmpl->addTemplate($ctmpl);
		$tmpl->addTemplate($vtmpl);
...
// 		$view->addPathway(JLText::getText('JL_PW_EDIT_TEAM_PROFILE'));
	...
	function uploadLogo() {
		...
		if (!JLSecurityService::isAuthorizedTask($team)) {
			$app->redirect("index.php?option=com_jleague&controller=teams&task=viewTeamProfile&teamid=" . $team->getSlug(), "Not authorized to Upload Team Logo Profile","error");
		}	
	...
	function manageSchedule() {
		...
		$message = null;
		try {
			$bool = $svc->validateTeamSchedule($teamid,$season->getId());
		} catch (Exception $e) {
			$message = $e->getMessage();
			$ttmpl->setObject("message", $message);
		}
// 		$ttmpl->setObject("message", $message);
...
// 		$tmpl->setObject('submenu2',$submenu2);
	...
	function manageContacts() {
		...
		$contacts = $svc->getTeamContacts($teamid, false);
	...
	//	$filepath = "/home/swibl/public_html" . DS . $folder.$filename;
...
// 		echo $tmp_name . "<br/>";
// 		echo $filepath . "<br/>";
// 		echo "SITE PATH: " . $app->getSitePath() . "<br/>";
// 		exit;
	...
	function doUpdate() {
		...
		//$req = fsRequest::getInstance();
		$teamid = self::getVariable("teamid",0);
...
// 		$mainframe->redirect( JRoute::_("index.php?option=com_jleague&controller=teams&task=viewTeamProfile&teamid=" . $teamid) , JLText::getText('JL_RECORD_SAVE_SUCCESSFUL') );
	...
	function addTeamVenue() {
		...
		//$req = fsRequest::getInstance();
		$teamid = self::getVariable("teamid");
		$venueid = self::getVariable("venueid");
	...
	function getGameHistory()
	{
		...
		$stmpl = new fsTemplate("teamprofile.view.schedule");
		$stmpl->setAlias("scheduleHtml");
		$stmpl->setObject("games", $svc->getTeamSchedule($teamid, $seasonid));
		$stmpl->setObject("team",$team);
		
		$view->addTemplate($stmpl);
		$view->render();
	}
	...
	function batchCalcStats() {
		...
		$seasonid = self::getVariable('seasonid');
		if ($seasonid == null) {
			echo "ERROR:  Missing SEASON ID value";
			return;
		}
		...
		if ($ssvc->isAdmin()) {
			$svc = & JLTeamService::getInstance();
			$svc->calculateTeamStats($seasonid);
		} else {
