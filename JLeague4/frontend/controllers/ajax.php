 ...
 */
class mAjax  extends mBaseController {
	...
	function doScheduleGame() {
		...
		try {
			$bool = $tservice->validateTeamSchedule($teamid,$season->getId());
		} catch (Exception $e) {
			$message = $e->getMessage();
			$tmpl->setObject("message", $message);
		}
...
// 		require_once(JLEAGUE_CLASSES_PATH . DS. 'helpers' . DS . 'json.class.php');
	...
	function doSavePlayerOnRoster() {
		...
		if ($teamid == null) {
			echo "AUTHENTICATION ERROR:  UNABLE TO IDENTIFY TEAM";
			return;
		}
		$security = &JLSecurityService::getInstance();
		if (!$security->canEditTeamProfileByTeamId($teamid)) {
			echo "AUTHENTICATION ERROR:  UNABLE TO SAVE TEAM CONTACT - NOT AUTHORIZED";
			return;
		}		
...
//			$view->display();
	...
	function doSaveBulletin() {
		...
		$bulletin->setCategory($req->getValue("bulletin_category"));
		...
		$bulletin->setSponsorId($req->getValue("sponsorid"));
		...
		$bulletin->setUpdatedBy($user->username);
	...
	function doSaveAdminBulletin() {
		$security = &JLSecurityService::getInstance();
		if (!$security->isAdmin()) {
			echo "AUTHENTICATION ERROR:  UNABLE TO SAVE BULLETIN - NOT AUTHORIZED";
			return;
		}
		...
		try {
			...
			if (_APPDEBUG) {
				$app->writeDebug("Error SAVING ADMIN BULLETIN", 2);
			}
			echo "ERROR:  Problem occurred saving Bulletin";
			return;
		}
...
// 		}
		...
		$bulletinid = $req->getValue("bulletinid");
		$teamid = $req->getValue("teamid");
		$sponsorid = $req->getValue("sponsorid");
		...
		switch ($bulletin->getCategory()) {
			case "S":
				$tname = "sponsor.bulletins.table";
				$bulletins = $service->getSponsorBulletins($sponsorid);
				break;
			case "A":
				$tname = "admin.bulletins.table";
				$bulletins = $service->getAdminBulletins();
				break;
			default:
				$tname = "teamprofile.bulletins.table";
				$bulletins = $service->getTeamBulletins($teamid);
		}
	...
	function doPromoteBulletin() {
		...
		$service = &JLBulletinsService::getInstance();
		try {
			$service->promoteBulletin($bulletinid, $teamid);
		} catch (Exception $e) {
			print_r($e);
			
			if (_APPDEBUG) {
				$app->writeDebug("Error PROMOTING BULLETIN", 2);
			}
			echo "ERROR:  Problem occurred promoting bulletin";
			return;
		}
