...
/**
  ...
  */  
function editTeamContact(id) {
	resetTeamContactForm(); 
	form = document.getElementById("team-contact-input-form");
	jfst.getJSON('index.php?option=com_jleague&tmpl=component&format=raw&controller=ajax&task=getTeamContactJSON',{ id: id },function(contact) {
		form.task.value='doSaveContact';
...
function editPlayerOnRoster(pid) {
	if (pid == 0) {
	//	resetPlayerForm();
	} else {}
		...
		jfst.getJSON('index.php?option=com_jleague&tmpl=component&format=raw&controller=ajax&task=getPlayerJSON&nodebug=1',{ playerid: pid },function(player) {
			form.task.value='savePlayerOnRoster';
...
function savePlayerOnRoster() {
	...
	if (confirm("Are you sure you wish to continue?")) {
		...
		jfst.ajax({
		    url: 'index.php?option=com_jleague&tmpl=component&format=raw&controller=ajax&task=doSavePlayerOnRoster',
		    data: str,
		    type: 'POST',
			dataType: 'html',
	   		error: function (XMLHttpRequest, textStatus, errorThrown) {
...
function postScore(id) {
	...
	jfst.getJSON('index.php?option=com_jleague&tmpl=component&format=raw&controller=ajax&task=getGameJSON',{ id: id },function(game) {
		...
		form.gamestatus_sl.selectedIndex = 2;
		form.gamestatus_sl.disabled = true;
