...
{
	var $redirectUrl = 'index.php?option=com_jleague&controller=registrations&task=display';
	...
	function display() {
		...
		$filter_season 	= $mainframe->getUserStateFromRequest( 'com_jleague.registrations.display'.'filter_season', 'filter_season', 0, 'int');
		$filter_divisionid 	= $mainframe->getUserStateFromRequest( 'com_jleague.registrations.display'.'filter_divisionid', 'filter_divisionid', 0, 'int');
		...
		$registrations = $service->getRecords($pagination->limitstart,$pagination->limit, ' ORDER BY season ', $filter);
