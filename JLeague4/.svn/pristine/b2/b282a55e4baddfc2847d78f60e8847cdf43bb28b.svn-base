 ...
 */
class JLeagueControllerStandings extends JLeagueController
{
	var $redirectUrl = 'index.php?option=com_jleague&controller=teams&task=listStandings';
	...
	function display() {
		...
		if ($filter_league > 0) {
			$filter[] = "leagueid = " . $filter_league;
		}
		if ($filter_season > 0) {
			$filter[] = "seasonid = " . $filter_season;
		}
		if ($filter_divisionid > 0) {
			$filter[] =  "divisionid = ". $filter_divisionid;
		}
