...
class JLStandingsDAO extends fsBaseDAO{
	...
	function getRecords($start, $stop, $orderby = '', $filter = '') {
		...
		$orderby = 'order by leagueid, divorder,  divisionid, seasonid, wins desc, runs_allowed asc ';
		if (sizeof($filter)>0) {
...
// 		require_once(JLEAGUE_CLASSES_PATH . DS .'helpers'. DS . 'factory.php');
		...
		$obj->setHeadCoach($row->headcoach);
		return $obj;
	}
	
	/**
	 *
	 * This function will insert a row into the TEMP STANDINGS table.
	 *
	 * @param JLStanding
	 * @return boolean
	 */
	function insertIntoTemp(JLStanding $obj) {
		$tablename = '#__jleague_temp_standings';
		$query = 'INSERT INTO ' .$this->getNameQuote( $tablename  ) . ' (id, league_id, season, division_id, position, team_id, teamname, wins, losses, ties, points, runs_scored, runs_allowed) '
				. ' VALUES (0,'
				. '"' . $obj->getLeagueId() . '",'
				. '"' . $obj->getSeason() . '",'
				. '"' . $obj->getDivisionId() . '",'
	...
	/**
	 * This function will delete ALL records from the Temp Standings table 
	 * 
	 */
	function deleteAllFromTemp() {
		$query = "delete from #__jleague_temp_standings where 1=1";
		try {
			$this->_updateRow($query);
		} catch (Exception $e) {
			throw $e;
		}
	}
	...
	/*
	 * This funciton updates rows in the temp table with the headcoach name
	 */
	function updateTempRecords() {
			$query = 'update #__jleague_temp_standings a
				left join joom_jleague_teams b on
    				a.team_id = b.id
				set
    				a.headcoach = b.coachname';
			return $this->_updateRow($query);
	}

	function getStandingsFromTemp($leagueid = null,$seasonid = null, $divisionid = null) {
		
		$filter = array();
		if ($leagueid > 0) {
			$filter[] = "s.league_id = " . $leagueid;
		}
		if ($seasonid > 0) {
			$filter[] = "s.season = " . $seasonid;
		}
		if ($divisionid > 0) {
			$filter[] = "s.division_id = " . $divisionid;
		}
	...
	//	$query = "SELECT * FROM `#__jleague_temp_standings` " . $cond; 

		
		$query = "SELECT s.*,l.id as leagueid,l.name as leaguename, d.id as divisionid, d.name as divisionname, d.sort_order as divorder, seasons.id as seasonid, seasons.title as seasontitle FROM `#__jleague_temp_standings` as s,
			`#__jleague_division` d, `#__jleague_leagues` l, `#__jleague_seasons` seasons
			 " . $where . " and s.league_id = l.id
				and s.division_id = d.id
				and s.season = seasons.id ";
		
		
		//$query = 'select * from #__jleague_temp_standings order by league_id, divorder,  divisionid, seasonid, position ';
		return parent::_getRows($query);
