...
class JLStandingsEngine {
	 ...
	 * @return array
	 */
	function generateStandings($season, $divid = null) {
		...
		foreach ($divisions as $division) {
			$divstdgs = $this->getDivisionalStandings($division->getId(),$season);
	...
	function generateTournamentStandings($season, $divid = null) {
	 ...
	 * @return array
	 */
	private function getDivisionalStandings($divid, $season) {
		$debug = false;
		$divsvc = JLDivisionService::getInstance();
    ...
    function retrieveTournamentData($id, $season, $leagueid=0) {
    	...
    	// =================================================================================
    	$query = "select * from ( "
    			. "\n select division_id, parent_divid as division_id, id, season, teamname, wins,losses, ties, wins+losses+ties total_games, ( wins * " . $winpts . ") + (losses * " . $losspts . ") + (ties * " . $tiepts . ") points, " .
    			" runs_scored, runs_allowed, (wins / (wins + losses + ties)) winpct from "
    			. "\n ( "
    			. "\n  select division_id, parent_divid, id, seasonid, teamname,sum(wins) wins,sum(losses) losses, sum(ties) ties, sum(points) points, sum(runs_scored) runs_scored, sum(runs_allowed) runs_allowed from"
    			. "\n ( "
    			. "\n select m.division_id, parent_divid, team.id, " . $season . " season, team.name teamname, 0 wins, 0 losses, 0 ties, 0 points, 0 runs_allowed, 0 runs_scored, 'nogame' game from #__jleague_divmap m, #__jleague_teams team where m.published = 1 	and season = " . $season . " and m.team_id = team.id "
    			. "\n UNION"
	...
	private function testHeadToHeadRecord(&$inArray,$season=null) {
		...
		.	' from #__jleague_scores score, #__jleague_teams team, #__jleague_divmap m  '
		.	' where score.hometeam_id = m.team_id and hometeam_id = team.id and hometeam_id in (' . $this->inClause . ') and  awayteam_id in (' . $this->inClause . ') and score.season = '.$season. ' and conference_game = "Y" and gamestatus = "C" '
		...
		.	' from #__jleague_scores score, #__jleague_teams team, #__jleague_divmap m  '
		.	' where score.awayteam_id = m.team_id and awayteam_id = team.id and hometeam_id in (' . $this->inClause . ') and  awayteam_id in (' . $this->inClause . ')  and score.season = '.$season. ' and conference_game = "Y" and gamestatus = "C" '
		.	' group by team.name, score.season '
	...
	private function getRunsAllowed($id) {
		...
		. 	' SELECT awayteam_id id,sum( awayteam_score ) runs_scored, sum( hometeam_score ) runs_allowed '
		.	' FROM #__jleague_scores WHERE awayteam_id = ' . $id
		.	' and conference_game = "Y" and gamestatus = "C" season = ' . $this->season . ' group by awayteam_id '
		.	' ) as summary group by id';
		$database->setQuery($query);
		$rows = $database->loadObjectList();
		if (sizeof($rows)>0) {
