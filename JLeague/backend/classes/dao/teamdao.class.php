<?php
/**
 * @version 		$Id: teamdao.class.php 468 2013-01-19 11:29:37Z cs1559 $
 * @package 		JLeague
...
class JLTeamDAO extends JLBaseDAO{
	...
	where dm1.division_id = d.id and dm1.published = 1 and dm1.season = 14
	order by team_id
	 */
...
// 			}
	...
	function getTeamsInSeason($season, $orderby=0,$registeredonly = true) {
		 ...
		 * 		$selectquery = "select * from (SELECT t.*, dm.season, dm.division_id, dm.agegroup, dm.registered
			...
			on t.id = dm.team_id) as tmptbl1 ";
		 */
		/* 04/11/2014 - SHUTDOWN CAUSED HERE */
		$selectquery = "select * from (SELECT t.*, dm.season, dm.division_id, dm.agegroup, dm.registered
			from #__jleague_teams t
			LEFT JOIN
			(
			...
			FROM `#__jleague_divmap` dm1,
				`#__jleague_division` d 
				where dm1.division_id = d.id and dm1.published = 1 and dm1.season = " . $season . "
				order by team_id
			) as dm
			on t.id = dm.team_id) as tmptbl1 ";
     ...
     * @return array 
     */
    function getTeamIdsForUser($id) {
    	if (!is_numeric($id)) {
			throw new  Exception("Userid Id is not numeric (TEAMDA0::getTeamIdsForUser)");
		}     	
    	$query = "SELECT id as teamid FROM `#__jleague_teams` WHERE ownerid = " . $id . " union select teamid from `#__jleague_teamcontacts` where userid = " . $id;
    ...
    function getOpponentsWinningPercentage($teamid, $season) {
    	$query = "select avg((s.wins / (s.losses + s.wins))) winpct from 
			(select distinct team_id from
			(SELECT hometeam_id as team_id
			FROM #__jleague_scores
			WHERE season = " . $season . " and conference_game = 'Y' and awayteam_id = " . $teamid ."
    ...
    function getNumberOfLeagueGames($teamid, $seasonid) {
    	...
    	(
    	SELECT count(*) as home_game_count  FROM `#__jleague_scores` WHERE `season` = " . $seasonid . " AND `hometeam_id` = ".$teamid." and conference_game = 'Y'
    	) as tmp1,
    	(
    	SELECT count(*) as away_game_count FROM `#__jleague_scores` WHERE `season` = " . $seasonid . " AND `awayteam_id` = ".$teamid." and conference_game = 'Y'
    	) as tmp2,
    	(
    	SELECT games from
    	#__jleague_division where id in (select division_id FROM `#__jleague_scores` WHERE `season` = " . $seasonid . " AND `hometeam_id` = ".$teamid." and conference_game = 'Y')
    	) as tmp3";
    	$rows = $this->_execute($query);
    	return $rows[0];
