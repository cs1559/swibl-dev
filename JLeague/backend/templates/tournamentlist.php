...
?>
			<tr>
				<td><?php echo $i + 1; ?></td>
				<td><?php  echo JHTML::_('grid.id', $i++, $tournament->getId()); ?></td>
				<td>
					<?php 
						$link		= JRoute::_( 'index.php?option=com_jtourney&controller=tournaments&task=edit&cid[]=' .$tournament->getId()  );
						echo JHTML::_('link', $link , $tournament->getName(), array('id' => $tournament->getId() )); 
					?>
				</td>
				<td>
					<?php echo $this->getPublish( $tournament , 'published'  );?>
				</td>
